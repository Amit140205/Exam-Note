...
function TopicForm({loading, setLoading, setResult, setError}) {
    ...
    const handleSubmit=async ()=>{
        ...
        try {
            const response=await generateNotes({topic, classLevel, examType, revisionMode, includeDiagram, includeChart})
            ...
            if(typeof response.creditsLeft==="number"){
                dispatch(updateCredit(response.creditsLeft))
            }
        } catch (error) {
            console.log(error)
  ...
  return (
    ...
    >
        <input type="text" className='w-full p-3 rounded-xl bg-white/10 backdrop-blur-lg border border-white/20 placeholder-gray-400 text-white focus:outline-none focus:ring-2 focus:ring-white/30'
        ...
        <input type="text" className='w-full p-3 rounded-xl bg-white/10 backdrop-blur-lg border border-white/20 placeholder-gray-400 text-white focus:outline-none focus:ring-2 focus:ring-white/30'
        placeholder='Class / Level (e.g. BTECH CSE)'
        onChange={(e)=>setClassLevel(e.target.value)}
        value={classLevel}
        />

        <input type="text" className='w-full p-3 rounded-xl bg-white/10 backdrop-blur-lg border border-white/20 placeholder-gray-400 text-white focus:outline-none focus:ring-2 focus:ring-white/30'
        placeholder='Exam Type (e.g. GATE, Semester Exam)'
        ...
        disabled={loading}
        className={`w-full mt-4 py-3 rounded-xl font-semibold flex items-start justify-center gap-3 transition 
        ${loading?"bg-gray-300 text-gray-600 cursor-not-allowed":"bg-linear-to-br from-white to-gray-200 text-black shadow-[0_15px_35px_rgba(0,0,0,0.4)] cursor-pointer"} 
        `}
        >
...
function Toggle({label, checked, onChange}){
    return(
        <div className='flex items-center gap-4 cursor-pointer select-none'>
            ...
            onClick={onChange}
            animate={{
                backgroundColor: checked? "rgba(34,197,94,0.35)":"rgba(255,255,255,0.15)"
            }}
